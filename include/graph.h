...
class constructing_graph {
  ...
  constructing_graph(std::array<std::string, sizeof...(Inputs)> names)
    : _nodes(util::make_std_vector<NodeVariant>(Source(make_type<Inputs>())...))
    , _names()
    , _outputs(sizeof...(Inputs)) {
 ...
 public:
  ...
  template <typename F>
  constructing_graph& add(F f, std::string name, small_vec<std::string, 3> inputs) {
    assert(!name_exists(name));

    auto node = InternalNode(make_any_function(f), util::map<small_vec<int, 3>>(inputs, [this](const std::string_view& name){ return idx_of(name); }));
  ...
  function_graph<Output, Inputs...> output(const std::string_view& name) {
    ...
    return function_graph<Output, Inputs...>(std::move(_nodes), std::move(_outputs));
  }

  template <typename... Ts>
  friend constructing_graph<Ts...> make_graph(std::array<std::string, sizeof...(Ts)> names);
};

template<typename... Inputs>
constructing_graph<Inputs...> make_graph(std::array<std::string, sizeof...(Inputs)> names) {
