...
class Type {
  ...
  public:
    ...
    Type(bool is_const, bool is_trivially_copyable, std::type_index index) :
      _is_const(is_const),
      _is_trivially_copyable(is_trivially_copyable),
      _index(index) { }
...
inline Type make_type() {
  return Type(std::is_const_v<T>, std::is_trivially_copyable_v<T>, std::type_index(typeid(T)));
}
...
inline small_vec<Type, 3> make_types() {
  return make_types_impl<TupleTypes>(std::make_index_sequence<std::tuple_size_v<TupleTypes>>());
}
