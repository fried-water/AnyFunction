...
BOOST_AUTO_TEST_CASE(any_basic) {
  ...
  BOOST_CHECK(any.has_value());
  BOOST_CHECK(type_id<int>() == any.type());
  BOOST_CHECK(5 == any_cast<int>(any));

  any = std::string("abc");

  BOOST_CHECK(any.has_value());
  BOOST_CHECK(type_id<std::string>() == any.type());
  BOOST_CHECK("abc" == any_cast<std::string>(any));

  const Any any2 = any;

  BOOST_CHECK(any2.has_value());
  BOOST_CHECK(type_id<std::string>() == any2.type());
...
BOOST_AUTO_TEST_CASE(any_move_only) {
  ...
  BOOST_CHECK(any.has_value());
  BOOST_CHECK(type_id<MoveOnlyType>() == any.type());
  ...
  BOOST_CHECK(!any.has_value());
  BOOST_CHECK(any2.has_value());
  BOOST_CHECK(type_id<MoveOnlyType>() == any2.type());
